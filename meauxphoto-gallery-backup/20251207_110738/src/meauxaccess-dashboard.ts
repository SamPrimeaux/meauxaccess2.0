...
import { GLASSMORPHIC_HEADER_HTML } from './glassmorphic-header.html.ts';
...
${LEGACY_HEADER_STYLES}
    ...
    // Glassmorphic header injection - replace logo URL
    let glassmorphicHeader = GLASSMORPHIC_HEADER_HTML.replace(
      /https:\/\/imagedelivery\.net\/[^"'\s]+/g,
      finalLogoUrl
    );

    if (html.includes('</head>')) {
      html = html.replace('</head>', `${glassmorphicHeader}\n</head>`);
    } else if (html.includes('<body')) {
      html = html.replace(/<body[^>]*>/, (match) => `${match}\n${glassmorphicHeader}`);
    } else {
      html = `${glassmorphicHeader}\n${html}`;
    }
  }
...
<body>
  ...
  <footer id="contact">
    <div class="container">
      ...
      <p style="margin-top: 1rem; opacity: 0.8;">Powered by Cloudflare Workers</p>
    </div>
  </footer>
</body>
</html>`;
